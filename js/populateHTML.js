...
function fetchBlogsFromMedium() {
	fetch(
		"https://api.rss2json.com/v1/api.json?rss_url=https://medium.com/feed/@chad1020"
	)
...
function populateProjects(items, id) {
	...
	for (let i = 0; i < items.length; i++) {
		...
		a.href = items[i].gitHubLink;
		a.target = "_blank";
		a.append(h4);

		let lnk = document.createElement("a");
		 lnk.href = items[i].preview;
		 lnk.target = "_blank";
		...
		divResumeContentLeft.append(lnk);
...
populateProjects(data.projects.IOs, "android-projects");
